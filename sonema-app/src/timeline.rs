        ...
        .show_viewport(ui, |ui, viewport| {
            ...
            for (track_index, track) in project.tracks.iter().enumerate() {
                ...
                painter.line_segment(
                    [
                        pos2(row_rect.left(), row_rect.bottom()),
                ...
                for clip in &track.clips {
                    let Some(duration) = project.clip_duration_frames(clip) else {
                        continue;
                    };
                    let left = world_rect.left()
                        + HEADER_WIDTH
                        + project.frames_to_seconds(clip.start_frame) as f32
                            * state.pixels_per_second;
                    let width = (project.frames_to_seconds(duration) as f32
                        * state.pixels_per_second)
            ...
            painter.line_segment(
                [
                    pos2(playhead_x, world_rect.top()),
                    pos2(playhead_x, world_rect.bottom()),
                ],
                Stroke::new(1.5, theme::ACCENT),
            );
            painter.circle_filled(
                pos2(playhead_x, world_rect.top() + RULER_HEIGHT - 5.0),
                4.0,
                theme::ACCENT,
            );
            ...
            if world_response.clicked() {
                if let Some(pointer) = world_response.interact_pointer_pos() {
                    ...
                    if pointer.x >= timeline_left {
                        let seconds =
                            ((pointer.x - timeline_left) / state.pixels_per_second).max(0.0);
...
fn begin_or_update_drag(
    ...
    let Some(drag) = state.drag.as_mut().filter(|drag| drag.clip == clip) else {
        return;
    };
    if response.dragged() {
        ...
        let apply_snap = |frame| {
            if no_snap {
                frame
            } else {
                snap.snap(project, frame)
            }
        };
        match drag.kind {
            DragKind::Move => {
...
fn draw_grid(
    ...
    for beat in first_beat..=last_beat {
        ...
        let color = if downbeat {
            theme::BORDER
        } else {
            Color32::from_rgb(34, 39, 43)
        };
        painter.line_segment(
...
fn draw_clip(
    ...
    if let Some(source) = source {
        ...
        for pixel in (0..width).step_by(step) {
            ...
            painter.line_segment(
                [
                    pos2(x, center - max * half_height),
                    pos2(x, center - min * half_height),
                ],
...
fn draw_sticky_headers(
    ...
    let header_rect =
        Rect::from_min_size(pos2(left, rect.top()), vec2(HEADER_WIDTH, rect.height()));
    ...
    for (index, track) in project.tracks.iter().enumerate() {
        ...
        for (label, active, action) in [
            ...
            if ui
                .interact(button, Id::new((label, track.id)), Sense::click())
                .clicked()
            {
                actions.push(action);
            }
    ...
    painter.line_segment(
        [
            pos2(left + HEADER_WIDTH, rect.top()),
            pos2(left + HEADER_WIDTH, rect.bottom()),
        ],
        Stroke::new(1.0, theme::BORDER),
    );
}
...
fn draw_drag_preview(
    ...
    let Some(track_index) = project
        .tracks
        .iter()
        .position(|track| track.id == drag.preview_track)
    else {
        return;
    };
    let left =
        rect.left() + HEADER_WIDTH + project.frames_to_seconds(drag.preview_start) as f32 * zoom;
    let width = project.frames_to_seconds(drag.preview_end.saturating_sub(drag.preview_start))
        as f32
        * zoom;
    ...
    painter.rect_filled(
        preview,
        4.0,
        Color32::from_rgba_unmultiplied(67, 213, 164, 48),
    );
