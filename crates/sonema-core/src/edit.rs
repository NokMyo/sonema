...
use crate::{Clip, ClipId, MediaId, MediaInfo, Project, TrackId, scale_frames};
...
impl Project {
    ...
    pub fn remove_clip(&mut self, clip_id: ClipId) -> Result<Clip, EditError> {
        let (track_index, clip_index) =
            self.clip_location(clip_id).ok_or(EditError::ClipNotFound)?;
    ...
    pub fn split_clip(&mut self, clip_id: ClipId, at_frame: u64) -> Result<ClipId, EditError> {
        ...
        let media = self
            ...
            .ok_or(EditError::MediaNotFound)?;
        let end = self
            .clip_end_frame(&original)
            .ok_or(EditError::MediaNotFound)?;
        if at_frame
            <= original
                .start_frame
                .saturating_add(MIN_CLIP_TIMELINE_FRAMES)
    ...
    pub fn trim_clip_start(&mut self, clip_id: ClipId, new_start: u64) -> Result<(), EditError> {
        ...
        let available_before =
            scale_frames(original.source_in, media.sample_rate, self.sample_rate);
        ...
        let source_delta = (timeline_delta as f64 * media.sample_rate as f64
